...
#PENDING TO VERIFY THE ADDED EXPORTATION OF THE REAL POINTS FROM THE VIRTUAL CAMERA
...
s_id = { 1: [3, 7, 5, 1] , 2 : [2, 6, 8, 4], 3 : [1, 5, 6, 2], 4 : [4, 8, 7, 3]} # SEQUENCES TO ORDER REFERENCE POINTS
#----------------------------------------------------------------------------------------------------------------------------------------------CLASS
#||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| 
...
def p2pix(rp, srv2, v, h):
    ...
    d = np.zeros((8,3)) # ARRAY TO STORE THE DATA, FORMAT: X,Y, POINT ID

    #/////////////////////////////////////////////////////#IMPORT THE 8 REFERENCE POINTS IN 3D COORDINATES (mm)-----------------------
    with open(rp) as r:                              

        ap = yaml.load(r, Loader=yaml.FullLoader) #ALL POINTS

    for i in range (8): #FOR THE 8 CALIB POINTS

        t   = 'p'+str(i+1)         #TARGET POINT ... 1,2,3,4,5,6,7,8     
        lst = ap.get(t)
    ...
    for s in range (len(s_id)): # <------------ SCANNING SEQUENCES (3)
        ...
        for i in range (4):    # 6 --- EXTRACT IN THE SEQUENTIAL ORDER THE LIST OF COORDS. X AND Y AND STORE THEM AT "out_str_x" & "out_str_y"
            ...
            for j in range(8): # SEARCH IN THE 8 ROWS OF POINT DATA
...
def expo(rp, srv, srv2, vc_kc, h_vir):
    ...
    op = '/home/sierra/oni_dir/oni_ws/src/oni_pkg/yaml_config/points4ExtCalib.yaml'  #OUTPUT PATH TO EXPORT THE .YAML FILE WITH THE REFERENCE POINTS
...
def cback(l,r,f,b):
    ...
    if getattr(srv, 'f_f') == 1:# & getattr(srv, 'f_f') == 0: #IF R IS READY GO TO F DETECTION...
        if getattr(srv, 'b_f') == 0:
            check(bdg.imgmsg_to_cv2(b,"bgr8") , 4) #DETECT IN RIGHT (Id:2) IMAGE USING THE FUNCTIONS "check" AND "detect" AND ALSO "load")            
...
if __name__ == "__main__":
    ...
    parser.add_argument('-rp', '--referencePointsPath',    type=str,  default='/home/sierra/oni_dir/oni_ws/src/oni_pkg/yaml_config/refPoints.yaml',      help='path, reference real points file') 
    ...
    try:
        ...
        sl  =  message_filters.Subscriber('/uImL', Image) # LEFT  SUBSCRIBER
        sr  =  message_filters.Subscriber('/uImR', Image) # RIGHT SUBSCRIBER
        sf  =  message_filters.Subscriber('/uImF', Image) # FRONT SUBSCRIBER
        sb  =  message_filters.Subscriber('/uImB', Image) # BACK SUBSCRIBER

        
        pub =  rospy.Publisher('/arucoIm', Image, queue_size=0.005) 
        
        syn = message_filters.ApproximateTimeSynchronizer([sl,sr, sf, sb], queue_size=1, slop=0.0005) #7 SYNC RETRIEVED DATA      
