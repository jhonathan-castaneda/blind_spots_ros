...
def get_rt_mtx(fc_kc, v_pix, v_cor, f_points, target, homography_out_path):
    ...
    #ESTIMATE AND SAVE HOMOGRAPHY MATRIX:-------------------------  NEW!
    ...
    b1 = [ v_pix[0,0]  , v_pix[1,0] ]
    b2 = [ v_pix[0,1]  , v_pix[1,1] ]
    b3 = [ v_pix[0,2]  , v_pix[1,2] ]
    b4 = [ v_pix[0,3]  , v_pix[1,3] ]
    
    pts_src      = np.array([s1, s2, s3, s4])
    pts_dst      = np.array([b1, b2, b3, b4])
    homo, status = cv2.findHomography(pts_src, pts_dst)
    
    with open(homography_out_path+target+'_homography_mtx.npy', 'wb') as m3:
        np.save(m3, homo)    
    #--------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c',  '--intrinsic_data',    type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/yaml_config/intrinsicData.yaml',   help='path, intrinsic data')    
    parser.add_argument('-p',  '--ref_calib_points',  type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/yaml_config/points4ExtCalib.yaml', help='path, reference points4calib')    
    parser.add_argument('-oe', '--out_path_ext',      type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/yaml_config/extrinsicData.yaml',   help='path, output path4ext calib') 
    ...
    parser.add_argument(     '--outHomographyPath', type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/data/homography/')
