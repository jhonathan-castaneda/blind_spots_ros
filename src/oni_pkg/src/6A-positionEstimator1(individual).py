...
def draw_bird_tracks(img, data):
    ...
    for i in range(len(data)): 
        ...
        cv2.circle(img, (   data[i,3] , data[i,2] ), 2, (255,255,255), 3)
        ...
        text       =  'TRACKING_ID: '+str(data[i,0])+'\n'+'   CLASS_ID: '+str(data[i,1])+'\n'+'   LOC:'+str(mmx)+','+str(mmy) 
        ...
        for idx, line  in enumerate(text.split('\n')):
            y = y0 + idx*dy
            cv2.putText(img, line, (x0,y), cv2.FONT_HERSHEY_SIMPLEX, 0.27, (255, 255, 255), 1)  
...
def upob1(dt, obs, rows):    # <<< INPUT: DATA ARRAY, OBSERVATOR OBJECT, NUMBER OF ROWS OF THE dt ARRAY (NUMBER OF TRACKS)
    ...
    for t in range(rows):
        ...
        sp     =  np.array([[ ut ],[ vt ],[ 1 ]])                                 # <<<  SURROUND PIXEL IN HOMOGENEOUS COORDS.
        vp1    =  np.matmul( lh, sp )                                             # <<<  VIRTUAL PIXEL IN HOMOGENEOUS COORDS(NOT NORMALIZED).
...
def deco(m1, m2):
    ...
    if m1.tracks is not []:
        ...
        if getattr(obs, 'bc') == 0:   # <<< IF NOT BROADCASTING
            ...
            img3     =  cv2.addWeighted(img2,0.5,  vm,1, 0)              
            m        =  bdg.cv2_to_imgmsg(img3, "mono8") 
            p.publish(m)
            #ONLY FOR DEBUG------------------------------------------1
