...
if __name__ == "__main__":
    ...
    parser.add_argument('-v','--virtualDataPath',  type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/yaml_config/virtualCam.yaml',  help='info of virtual cam')
        
    parser.add_argument('-e','--extrinsicDataPath',  type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/yaml_config/extrinsicData.yaml',  help='extrinsic calib data')
    
    parser.add_argument('-i','--intrinsicDataPath',  type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/yaml_config/intrinsicData.yaml',  help='intrinsic calib data')

    parser.add_argument('-m','--vehicleMaskPath',  type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/data/image/mask_8000x8000_500x500.png',  help='vehicle mask')
    
    parser.add_argument('-o','--outPathForMaps',   type=str,  default='/home/oni/oni_dir/oni_ws/src/oni_pkg/data/wrapping_maps/')
    ...
    try:
        ...
        #CHECK IF THE PIXEL MAPS DO EXIST, OTHERWHISE CALCULATE THEM OR THE REMAINING ONES
        #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        
        if exists(args.outPathForMaps+'l_wrappingMap.npy'):
            with open(args.outPathForMaps+'l_wrappingMap.npy', 'rb') as s1:
                lw = np.load(s1)
        else:
            lrt,  lkc     =  rt[:,:,0]  ,  fc[:,:,0]        
            lw            =  wmap(h_vir, vki, lrt, lkc, m0) #LEFT WRAPPING MAP AND LEFT MASK
            
            with open(args.outPathForMaps+'l_wrappingMap.npy', 'wb') as m1:
                np.save(m1, lw)            
        #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        
        if exists(args.outPathForMaps+'r_wrappingMap.npy'):
            with open(args.outPathForMaps+'r_wrappingMap.npy', 'rb') as s2:
                rw = np.load(s2)
        else:         
            rrt,  rkc     =  rt[:,:,1]  ,  fc[:,:,1]        
            rw            =  wmap(h_vir, vki, rrt, rkc, m0) #RIGHT WRAPPING MAP AND RIGHT MASK
            
            with open(args.outPathForMaps+'r_wrappingMap.npy', 'wb') as m2:
                np.save(m2, rw)                   
        #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        
        if exists(args.outPathForMaps+'f_wrappingMap.npy'):
            with open(args.outPathForMaps+'f_wrappingMap.npy', 'rb') as s3:
                fw = np.load(s3)
        else:           
            frt,  fkc     =  rt[:,:,2]  ,  fc[:,:,2]        
            fw            =  wmap(h_vir, vki, frt, fkc, m0) #FRONT WRAPPING MAP AND FRONT MASK
            
            with open(args.outPathForMaps+'f_wrappingMap.npy', 'wb') as m3:
                np.save(m3, fw)            
        #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

        if exists(args.outPathForMaps+'b_wrappingMap.npy'):
            with open(args.outPathForMaps+'b_wrappingMap.npy', 'rb') as s4:
                bw = np.load(s4)
        else:
            brt,  bkc     =  rt[:,:,3]  ,  fc[:,:,3]        
            bw            =  wmap(h_vir, vki, brt, bkc, m0) #BACK WRAPPING MAP AND BACK MASK
            
            with open(args.outPathForMaps+'b_wrappingMap.npy', 'wb') as m4:
                np.save(m4, bw)
        #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

        rospy.loginfo('PIXEL MAPS READY AND STORED AT:  '+args.outPathForMaps+'\n'+'VIRTUAL VIEW GENERATION IS NOW AVAILABLE.')    #  -- REGISTER THE INITIALIZATION OF THE NODE
