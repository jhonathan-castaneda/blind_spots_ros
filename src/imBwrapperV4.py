...
def brd(m):
    ...
    l2  = cv2.transpose(m1u[:,:,0])
    r2 = cv2.transpose(m1u[:,:,1])
    f2 = cv2.transpose(m1u[:,:,2])
    b2  = cv2.transpose(m1u[:,:,3])

    o = cv2.addWeighted(f2,0.3,l2,0.4, 0)
    o = cv2.addWeighted(o,1,r2,0.3, 0)
    o = cv2.addWeighted(o,1,b2 ,0.3, 0)
