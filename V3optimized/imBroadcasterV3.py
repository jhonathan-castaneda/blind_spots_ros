...
def corr(ms):
    ...
    cel,  cgl  = ms.corr_e_l,  ms.corr_g_l  #CORRECTION_EXPOSURE_L,  CORRECTION_GAIN_L   
    cer,  cgr  = ms.corr_e_r,  ms.corr_g_r  #CORRECTION_EXPOSURE_R,  CORRECTION_GAIN_R
    cef,  cgf  = ms.corr_e_f,  ms.corr_g_f  #CORRECTION_EXPOSURE_F,  CORRECTION_GAIN_F    
...
def bct():
    ...
    pl   = rospy.Publisher('/dImL', Image, queue_size=0.0005)    # LEFT PUBLISHER
    pr   = rospy.Publisher('/dImR', Image, queue_size=0.0005)    # RIGHT PUBLISHER
    pf   = rospy.Publisher('/dImF', Image, queue_size=0.0005)    # FRONT PUBLISHER
   #pb   = rospy.Publisher('/dImB', Image, queue_size=0.04)      # BACK PUBLISHER
    
    pcs  = rospy.Publisher('/cStat',cams_status, queue_size=0.0005)    # (PUBLISHER OF CAMERAS STATUS)
    cs   = rospy.Subscriber('/fCtrl', fuzzy_corr, corr)                # (FUZZY CORRECTION SUBSCRIBER)   
    mcs  = cams_status()                                               # 5 -- INITIALIZE CAMS STATUS MSG (CAMS_STATUS OBJECT) 
    ...
    while not rospy.is_shutdown():   # MANI SEQUENCE------------------------------------------
       ...
       #mb = bdg.cv2_to_imgmsg(ffb, "bgr8")   #MSG BACK
        ...
        mcs.gain_l = int(l.get(cv2.CAP_PROP_GAIN)  )      ;   mcs.exp_l  = int(l.get(cv2.CAP_PROP_EXPOSURE) )   #   10 -- ORGANIZE LEFT STATUS IN MSG
        mcs.gain_r = int(r.get(cv2.CAP_PROP_GAIN) )       ;   mcs.exp_r  = int(r.get(cv2.CAP_PROP_EXPOSURE) )   #   11 -- ORGANIZE RIGHT STATUS IN MSG
        mcs.gain_f = int(f.get(cv2.CAP_PROP_GAIN) )       ;   mcs.exp_f  = int(f.get(cv2.CAP_PROP_EXPOSURE) )   #   12 -- ORGANIZE FRONT STATUS IN MSG
